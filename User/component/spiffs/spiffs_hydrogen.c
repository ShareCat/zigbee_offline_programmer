...
spiffs_file SPIFFS_open(spiffs *fs, const char *path, spiffs_flags flags, spiffs_mode mode)
{
    ...
    /* @如果不允许创建文件，那本函数返回，因为假设准备打开的文件，在文件系统里面没
        有找到，那么SPIFFS_open函数打开文件就没有意义，因此直接函数返回 */
    ...
    /* @如果SPIFFS_O_CREAT和SPIFFS_O_EXCL都置位了，并且要打开的文件存在，
        SPIFFS_open就会失败 */
    if(res == SPIFFS_OK &&
    ...
    /* @如果要打开的文件不存在，并且允许创建文件，既然不存在那就创建这个文件 */
    if((flags & SPIFFS_O_CREAT) && res == SPIFFS_ERR_NOT_FOUND) {
#if !SPIFFS_READ_ONLY   /* 如果SPIFFS文件系统只读，那么就不要创建文件了 */
        spiffs_obj_id obj_id;
