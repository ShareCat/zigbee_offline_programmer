...
#define SPIFFS_OBJ_ID_DELETED           ((spiffs_obj_id)0)  /* obj_id = 0：用于表示页面已经被删除 */
#define SPIFFS_OBJ_ID_FREE              ((spiffs_obj_id)-1) /* obj_id = 0xffff：用于表示页面为空闲页面 */
...
/*
    先看这个：https://blog.csdn.net/zhangjinxing_2006/article/details/75050611

   @SPIFFS文件系统存储数据最小单位是page，也就是说即使你只要存储一个字节，也需
    要花费1个page来存储，另外每个page前面有关于这个page存储信息的元数据，元数
    据有3个分别是obj_id:表示属于哪个文件；span_ix:表示这是第几个数据page，因为
    有时候一个文件需要多个数据page来存储；flags:用于描述本数据page的情况，如这
    个数据page存储的是【文件数据】还是【文件头】，这个数据page是否被使用或是空
    闲或被删除等，什么是元数据自行百度~，简单一句话就是元数据是描述数据的数据

   @上面说到了：一个数据page可以存储：【文件数据】或是【文件头】，【文件数据】
    就是一个文件有效的数据，【文件头】是这个文件的信息，包括文件名字，文件总字
    节数，存储文件消耗的page个数，等等

   @假设使用的SPI_FLASH一个block为64kB，一个page为256B，所以每个block里面有256
    个page，SPIFFS文件系统中每个block里面分对象page和数据page，对象page用于指出
    本block中所有其余页所属的对象ID，如果一个ID占2个字节，所以一个对象page可以
    实现对128个数据page分别存储的文件的ID，所以每个block里面需要2个索引page，剩
    下254个page为数据page，关于对象page看下面

   @对象page：使用spiffs_obj_id来描述每个数据page到底属于哪个文件（ID），的具体
    看spiffs_obj_id的定义，目前定义是u16_t，最高有效位被置位表示该页为一个索引
    页，最高有效位为0表示该页为一个数据页

   边阅读代码，边做笔记----孙关平20180404 */

// total number of pages
#define SPIFFS_MAX_PAGES(fs) \
  ( SPIFFS_CFG_PHYS_SZ(fs)/SPIFFS_CFG_LOG_PAGE_SZ(fs) )         /* 分配给SPIFFS文件系统的page总数 */
// total number of pages per block, including object lookup pages
#define SPIFFS_PAGES_PER_BLOCK(fs) \
  ( SPIFFS_CFG_LOG_BLOCK_SZ(fs)/SPIFFS_CFG_LOG_PAGE_SZ(fs) )    /* =256，block为64kB，page为256B来算 */
// number of object lookup pages per block
#define SPIFFS_OBJ_LOOKUP_PAGES(fs)     \
  (MAX(1, (SPIFFS_PAGES_PER_BLOCK(fs) * sizeof(spiffs_obj_id)) / SPIFFS_CFG_LOG_PAGE_SZ(fs)) )  /* =2 */
// checks if page index belongs to object lookup
#define SPIFFS_IS_LOOKUP_PAGE(fs,pix)     \
  (((pix) % SPIFFS_PAGES_PER_BLOCK(fs)) < SPIFFS_OBJ_LOOKUP_PAGES(fs)) /* 判断pix是否是索引page，每个block的前2个page为索引page */
// number of object lookup entries in all object lookup pages
#define SPIFFS_OBJ_LOOKUP_MAX_ENTRIES(fs) \
  (SPIFFS_PAGES_PER_BLOCK(fs)-SPIFFS_OBJ_LOOKUP_PAGES(fs))              /* =254 */
// converts a block to physical address
#define SPIFFS_BLOCK_TO_PADDR(fs, block) \
  ( SPIFFS_CFG_PHYS_ADDR(fs) + (block)* SPIFFS_CFG_LOG_BLOCK_SZ(fs) )   /* 根据block编号，计算物理地址 */
...
#define SPIFFS_PAGE_TO_PADDR(fs, page) \
  ( SPIFFS_CFG_PHYS_ADDR(fs) + (page) * SPIFFS_CFG_LOG_PAGE_SZ(fs) )    /* 根据page编号，计算物理地址 */
// converts a physical address to page
#define SPIFFS_PADDR_TO_PAGE(fs, addr) \
  ( ((addr) -  SPIFFS_CFG_PHYS_ADDR(fs)) / SPIFFS_CFG_LOG_PAGE_SZ(fs) ) /* 根据物理地址，计算page编号 */
// gives index in page for a physical address
#define SPIFFS_PADDR_TO_PAGE_OFFSET(fs, addr) \
  ( ((addr) - SPIFFS_CFG_PHYS_ADDR(fs)) % SPIFFS_CFG_LOG_PAGE_SZ(fs) )  /* 计算物理地址对page的偏移量 */
// returns containing block for given page
#define SPIFFS_BLOCK_FOR_PAGE(fs, page) \
  ( (page) / SPIFFS_PAGES_PER_BLOCK(fs) )                               /* 根据page编号，计算这个page属于哪个block */
// returns starting page for block
#define SPIFFS_PAGE_FOR_BLOCK(fs, block) \
  ( (block) * SPIFFS_PAGES_PER_BLOCK(fs) )                              /* 根据block编号，计算该block的第一个page编号 */
// converts page to entry in object lookup page
#define SPIFFS_OBJ_LOOKUP_ENTRY_FOR_PAGE(fs, page) \
  ( (page) % SPIFFS_PAGES_PER_BLOCK(fs) - SPIFFS_OBJ_LOOKUP_PAGES(fs) ) /* 根据page编号，转化为具体某个block里面的数据page编号 */
// returns data size in a data page
#define SPIFFS_DATA_PAGE_SIZE(fs) \
    ( SPIFFS_CFG_LOG_PAGE_SZ(fs) - sizeof(spiffs_page_header) )         /* 一个数据page可用的有效字节个数 */
