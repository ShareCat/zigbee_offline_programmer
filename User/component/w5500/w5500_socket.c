...
#define DEBUG_W5500_SOCKET FALSE        /* 调试开关 */
...
uint8_t socket(SOCKET s, uint8_t protocol, uint16_t port, uint8_t flag)
{
    ...
    if(((protocol & 0x0F) == Sn_MR_TCP)    ||
        ...
        if(port != 0) {
            ...
            w5500_config.lport++;   /* If don't set the source port, set local_port number */
            IINCHIP_WRITE(Sn_PORT0(s), (uint8_t)((w5500_config.lport & 0xff00) >> 8));
            IINCHIP_WRITE(Sn_PORT1(s), (uint8_t)(w5500_config.lport & 0x00ff));
        }
...
/**
 * @brief    查询connect_no_check函数连接的结果，和connect_no_check函数配合使用
 * @param    s: socket number.
 * @return   连接超时，返回0；正在连接，返回2；连接成功，返回1
 */
uint8_t connect_check(SOCKET s)
{
    uint8_t ret;
...
*@return    1 for success else 0.
*/
uint8_t connect_no_check(SOCKET s, uint8_t * addr, uint16_t port)
{
    uint8_t ret;
...
//这里while有可能一直等待最长会持续1600ms，可以不用sendto函数，换成sendto_no_check加sendto_check函数方式
        //aaa = 0;
        /* 查询是否连接成功 */
...
/**
 * @brief    查询sendto_no_check函数发送的结果，和sendto_no_check函数配合使用
 * @param    s: socket number.
 * @return   发送超时，返回0；正在发送，返回2；发送成功，返回1
 */
uint8_t sendto_check(SOCKET s)
{
    uint8_t ret = 0;

    /* 查询是否发送完成 */
    if((IINCHIP_READ(Sn_IR(s)) & Sn_IR_SEND_OK) != Sn_IR_SEND_OK) {
        if(IINCHIP_READ(Sn_IR(s)) & Sn_IR_TIMEOUT) {
            IINCHIP_WRITE(Sn_IR(s), (Sn_IR_SEND_OK | Sn_IR_TIMEOUT));   /* Clear SEND_OK & TIMEOUT */
            ret = 0;    /* 发送超时，返回0，也代表发送失败了 */
#if DEBUG_W5500_SOCKET
            PRINTF("sendto_check fail\r\n");
#endif
        } else {
            ret = 2;    /* 正在发送，返回2 */
#if DEBUG_W5500_SOCKET
            PRINTF("sendto_check wait\r\n");
#endif
        }
    } else {
        ret = 1;        /* 发送成功，返回1 */
#if DEBUG_W5500_SOCKET
        PRINTF("sendto_check ok\r\n");
#endif
        IINCHIP_WRITE(Sn_IR(s), Sn_IR_SEND_OK);
    }
...
*/
uint16_t sendto_no_check(SOCKET s, const uint8_t* buf, uint16_t len, uint8_t* addr, uint16_t port)
{
    uint16_t ret = 0;
